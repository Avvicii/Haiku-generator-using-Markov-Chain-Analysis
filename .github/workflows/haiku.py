...
def map_words(corpus):
    ...
    for index, word in enumerate(corpus):
        if index < limit:
            ...
            dict[word].append(suffix)

    logging.debug("results for map_words for \"sake\" = %s\n", dict['sake'])
    return dict

def map_2words(corpus):
    limit = len(corpus) - 2
    ...
    for index, word in enumerate(corpus):
        if index < limit:
            ...
            dict2[key].append(suffix)

    logging.debug("results for map_words for \"sake jug\" = %s\n", dict2.get('sake jug', []))
    return dict2
...
def random_word(corpus):
    ...
    if syllables > 4:
        ...
        logging.debug("random word & syllabus = %s %s \n", word, syllables)
...
def haiku_line(suffix_map_1, suffix_map_2, corpus, end_prev_line, target_syls):
    ...
    if len(end_prev_line) == 0:     #build line 1
        line = '1'
        ...
        nums_syls = count_syllables(word)
        logging.debug("words & syllables = %s %s", word, num_syls)
        line_syls += num_syls
        current_line.append(word)
...
def main():
    ...
    suffix_map_1 = map_words(corpus)
    suffix_map_2 = map_2words(corpus)
    final = []
